 ...
 */
 ...
 static int derefine_edge(int i);
 ...
 /*! \brief de-refinement criterion at edges of the box
  *
  * Includes the default de-refinement criterion
  * Checks if a given cell is within 5% of the box edge in any axis. Marks for derefinement if 
  * edge cell is <= than 15% mean volume
  *
  *  \param[in] i Index of cell in P and SphP arrays.
  *
  *  \return Flag if this cell should be dissolved.
 ...
 static int derefine_edge(int i)
 {
 #if defined(REFINEMENT_SPLIT_CELLS) && defined(REFINEMENT_MERGE_CELLS) && defined(EDGE_DEREFINEMENT)
   if(P[i].Mass < 0.5 * All.TargetGasMass)
     return 1;
 
   if (P[i].Pos[0] >= 0.95*All.BoxSize || P[i].Pos[1] >= 0.95*All.BoxSize || P[i].Pos[2] >= 0.95*All.BoxSize
   || P[i].Pos[0] <= 0.05*All.BoxSize || P[i].Pos[1] <= 0.05*All.BoxSize || P[i].Pos[2] <= 0.05*All.BoxSize)
   {   
     if (SphP[i].Volume < 0.15 * All.MeanVolume) 
       // printf("Marking cell for de-referencing at position: %f | %f | %f \n", P[i].Pos[0], P[i].Pos[1], P[i].Pos[2]);
       return 1;
   }
 #endif /* #ifdef EDGE_DEREFINEMENT */
   return 0;
 }
