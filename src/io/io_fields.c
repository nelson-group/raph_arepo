...
static void io_func_coolrate(int particle, int components, void *buffer, int mode)
{
  ...
  if (fabs(rad_i) <= All.BoxLimit && fabs(rad_j ) <= All.BoxLimit && fabs(rad_k) <= All.BoxLimit)
  {
    ...
    ((MyOutputFloat *)buffer)[0] = -coolrate; // By convention, we set heating to negative and cooling to positive 
  }
  else 
...
void init_io_fields()
{
  ...
  init_units(IO_CT, 0., -1., 1., 0., -1., All.UnitTime_in_s);
